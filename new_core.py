...
def is_white_background(mamm, threshold=128):
    # Put image on grayscale int 0-255
    mamm_restored = (mamm * 255).astype(np.uint8)

    # Histogram
    hist = cv2.calcHist([mamm_restored], [0], None, [256], [0, 256])
    ...
    threshold_intensity = np.argmax(cum_sum >= (threshold / 256.0))
    is_white_bg = threshold_intensity >= 128
